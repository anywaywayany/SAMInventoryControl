...
import com.samic.samic.exceptions.ProducerException;
...
public class ServiceProducer{
    ...
    public Producer saveProducerByObject(Producer producer){
        if(producer != null){
                ...
                if(doesObjectExistById(producer.getId())){
                    ...
                    if(objectById != null){
                        if(objectById.getId().equals(producer.getId())){
                            ...
                            throw new ProducerException("Producer with id1: '%s' and id2: '%s' does not match. Some error occoured while fetch!!".formatted(objectById.getId(), producer.getId()));
                        }
                    }else{
                        throw new ProducerException("Producer with id: '%s' does not exist in DB".formatted(producer.getId()));
                    }
                }else{
                    throw new ProducerException("Producer with id: '%s' does not exist in DB but does have a id: ".formatted(producer.getId()));
                }
            }else{
            ...
            throw new ProducerException("Producer is null!");
        }
    }
    ...
    public Producer findProducerById(Long id){
        if(id != null){
            if(repositoryProducer.findById(id).isPresent()){
                ...
                throw new ProducerException("Could not find Producer with id: '%s' in DB".formatted(id));
            }
        }else{
            throw new ProducerException("Given id is null!");
        }
    }
    ...
    public void deleteByObject(Producer producer){
        if(producer != null){
            ...
            throw new ProducerException("Given Producer is null!");
        }
    }
    ...
    public Optional<Producer> findProducerByNameOptional(String name){
        if(name != null){
            if(repositoryProducer.findStorageByName(name).isPresent()){
                ...
                throw new ProducerException("Could not find Producer with name: '%s' in DB".formatted(name));
            }
        }else{
            throw new ProducerException("Given name is null!");
        }
    }
